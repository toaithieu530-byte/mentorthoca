  ...
  const playNextAudio = async () => {
    ...
    if (task.isReady && task.base64Audio) {
      ...
      try {
        ...
        if (/NotAllowedError|play\(\) failed|user (didn't|did not) interact/i.test(message)) {
          setTtsError('Trình duyệt đang chặn tự phát âm thanh. Bạn bấm nút loa ở góc trên để nghe lại đoạn thơ.');
        }
      } finally {
